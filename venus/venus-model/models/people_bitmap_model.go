...
type FrequentCustomerPeople struct {
	...
	LastCaptureAt           time.Time `gorm:"type:timestamp with time zone"`
...
func (person *FrequentCustomerPeople) UpdateBitMap(tx *gorm.DB, personID string, today time.Time) (FrequentCustomerPeopleBitMap, error) {
	var bitMap FrequentCustomerPeopleBitMap
	tx.Preload("FrequentCustomerPeople").
	...
	if bitMap.ID == 0 || len(bitMap.BitMap) != 32 { // 以前从来没来过
		...
		lastDate := currentTime(bitMap.FrequentCustomerPeople.Hour, "day")
		...
		err := tx.Save(&newBitMap).Error
		return newBitMap, err
	}
}

// UpdateValueWithBitMap 根据bitMap，person的数据得到更新
func (person *FrequentCustomerPeople) UpdateValueWithBitMap(tx *gorm.DB, bitMap *FrequentCustomerPeopleBitMap, group *FrequentCustomerGroup) {
...
func (person *FrequentCustomerPeople) GetType(tx *gorm.DB) string {
	...
	if person.Frequency <= 1 { // 一次以内就不算回头客
		...
		tx.Where("company_id = ?", person.FrequentCustomerGroup.CompanyID).First(&rule)
...
func (person *FrequentCustomerPeople) IsHighFrequency(tx *gorm.DB) bool {
	return person.GetType(tx) == FREQUENT_CUSTOMER_TYPE_HIGH
}
